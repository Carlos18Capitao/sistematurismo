...
const props = defineProps({ tour: Object, categories: Array, difficulties: Array, provinces: Array });

const highlights = ref([...( props.tour.highlights ?? [])]);
const includes = ref([...(props.tour.includes ?? [])]);
const excludes = ref([...(props.tour.excludes ?? [])]);
...
const form = useForm({
    ...
    available_spots: props.tour.available_spots,
    category: props.tour.category,
    difficulty: props.tour.difficulty,
    available_from: props.tour.available_from ? props.tour.available_from.slice(0, 10) : '',
    available_until: props.tour.available_until ? props.tour.available_until.slice(0, 10) : '',
    cover_image: null,
...
function submit() {
    syncArrays();
    form.post(route('admin.tours.update', props.tour.id));
}
</script>

<template>
    <Head title="Editar Tour" />
        ...
        <form @submit.prevent="submit" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <div v-if="tour.cover_image" class="mb-2">
                        <img :src="`/storage/${tour.cover_image}`" alt="Capa atual" class="h-28 w-48 object-cover rounded-xl border border-gray-200">
